...
package de.unratedfilms.guilib.vanilla.sliders;

import net.minecraft.util.MathHelper;
import de.unratedfilms.guilib.core.Slider;
import de.unratedfilms.guilib.vanilla.SliderVanilla;
...
public class IntSlider extends SliderVanilla {
    ...
    public boolean mouseWheel(int delta) {
        ...
        if (hover && !dragging) {
            setValue(getFloatValue(getIntValue() + (int) Math.signum(delta), minVal, maxVal));
            return true;
        }
        return false;
    }
    ...
    public void setIntValue(int val) {
        ...
        setValue(MathHelper.clamp_float(getFloatValue(val, minVal, maxVal), 0, 1));
    }

    public int getIntValue() {

        return Math.round(getValue() * (maxVal - minVal) + minVal);
    }
